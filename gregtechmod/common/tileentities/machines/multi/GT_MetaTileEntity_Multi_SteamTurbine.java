...
public class GT_MetaTileEntity_Multi_SteamTurbine extends GT_MetaTileEntity_MultiBlockBase {
   ...
   public void onRightclick(EntityPlayer aPlayer) {
      this.getBaseMetaTileEntity().openGUI(aPlayer, 157, GregTech_API.gregtechmod);
   }

   public GT_MetaTileEntity_Multi_SteamTurbine(int aID, String aName, String aNameRegional) {
   ...
   public boolean checkMachine(ItemStack aStack) {
      ...
      if(this.getBaseMetaTileEntity().getAirAtSideAndDistance(this.getBaseMetaTileEntity().getBackFacing(), 1) && this.getBaseMetaTileEntity().getAirAtSideAndDistance(this.getBaseMetaTileEntity().getBackFacing(), 2)) {
         ...
         if(tAirCount != 10) {
            ...
            for(tX = 2; tX < 6; ++tX) {
               if(null != (var11 = this.getBaseMetaTileEntity().getIGregTechTileEntityAtSideAndDistance(tX, 2)) && var11.getFrontFacing() == this.getBaseMetaTileEntity().getFrontFacing() && var11.getMetaTileEntity() != null && var11.getMetaTileEntity() instanceof GT_MetaTileEntity_Multi_SteamTurbine) {
                  return false;
               }
            }
            ...
            for(byte i = -1; i < 2; ++i) {
               for(byte j = -1; j < 2; ++j) {
                  if(i != 0 || j != 0) {
                     for(byte k = 0; k < 4; ++k) {
                        if((i == 0 || j == 0) && (k == 1 || k == 2)) {
                           if((this.getBaseMetaTileEntity().getBlock(var12 + (tSide < 4?i:(tSide == 5?k:-k)), var13 + j, tZ + (tSide < 4?(tSide == 3?k:-k):i)) != GregTech_API.sBlockList[0] || this.getBaseMetaTileEntity().getMetaID(var12 + (tSide < 4?i:(tSide == 5?k:-k)), var13 + j, tZ + (tSide < 4?(tSide == 3?k:-k):i)) != 13) && !this.addToMachineList(this.getBaseMetaTileEntity().getIGregTechTileEntity(var12 + (tSide < 4?i:(tSide == 5?k:-k)), var13 + j, tZ + (tSide < 4?(tSide == 3?k:-k):i)))) {
                              return false;
                           }
                        } else if(this.getBaseMetaTileEntity().getBlock(var12 + (tSide < 4?i:(tSide == 5?k:-k)), var13 + j, tZ + (tSide < 4?(tSide == 3?k:-k):i)) != GregTech_API.sBlockList[0] || this.getBaseMetaTileEntity().getMetaID(var12 + (tSide < 4?i:(tSide == 5?k:-k)), var13 + j, tZ + (tSide < 4?(tSide == 3?k:-k):i)) != 13) {
                           return false;
                        }
                     }
                  }
   ...
   public Icon getTextureIcon(byte aSide, byte aFacing, boolean aActive, boolean aRedstone) {
      return aSide == aFacing?(aActive?GT_BlockMetaID_Block.mIconSteamTurbineActive[4]:GT_BlockMetaID_Block.mIconSteamTurbine[4]):null;
   }
