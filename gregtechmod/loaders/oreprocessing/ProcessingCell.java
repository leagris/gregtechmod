   ...
   public void registerOre(OrePrefixes aPrefix, Materials aMaterial, String aOreDictName, String aModName, ItemStack aStack) {
      if(aMaterial == Materials.Empty) {
         ...
         if(aMaterial.mFuelPower > 0) {
            GregTech_API.sRecipeAdder.addFuel(GT_Utility.copyAmount(1L, new Object[]{aStack}), GT_Utility.getFluidForFilledItem(aStack) == null?GT_Utility.getContainerItem(aStack):null, aMaterial.mFuelPower, aMaterial.mFuelType);
         }

         if(aMaterial == Materials.Ice) {
            GT_ModHandler.addSmeltingAndAlloySmeltingRecipe(GT_Utility.copyAmount(1L, new Object[]{aStack}), GT_OreDictUnificator.get(OrePrefixes.cell, (Object)Materials.Water, 1L));
         }
         ...
         if(aMaterial.mMaterialList.size() > 0 && (aMaterial.mExtraData & 3) != 0) {
            ...
            MaterialStack tMat;
            for(Iterator tItemAmount = aMaterial.mMaterialList.iterator(); tItemAmount.hasNext(); tAllAmount = (int)((long)tAllAmount + tMat.mAmount)) {
            ...
            Iterator i$ = aMaterial.mMaterialList.iterator();

            while(i$.hasNext()) {
               MaterialStack tMat1 = (MaterialStack)i$.next();
               if(tMat1.mAmount > 0L) {
                  ItemStack tStack;
                  ...
                  if(tItemAmount1 + tMat1.mAmount * 3628800L <= (long)aStack.func_77976_d() * aMaterial.getDensity()) {
                     tItemAmount1 += tMat1.mAmount * 3628800L;
                     if(tStack != null) {
                        for(tStack.field_77994_a = (int)((long)tStack.field_77994_a * tDensityMultiplier); tStack.field_77994_a > 64; tStack.field_77994_a -= 64) {
                        ...
                        if(tStack.field_77994_a > 0 && tCapsuleCount + (long)GT_ModHandler.getCapsuleCellContainerCountMultipliedWithStackSize(new ItemStack[]{tStack}) <= 64L) {
            ...
            if(tList.size() > 0) {
               if((aMaterial.mExtraData & 1) != 0) {
                  GregTech_API.sRecipeAdder.addElectrolyzerRecipe(GT_Utility.copyAmount(tItemAmount1, new Object[]{aStack}), tCapsuleCount > 0L?(int)tCapsuleCount:0, (ItemStack)tList.get(0), tList.size() < 2?null:(ItemStack)tList.get(1), tList.size() < 3?null:(ItemStack)tList.get(2), tCapsuleCount < 0L?GT_Items.Cell_Empty.get(-tCapsuleCount, new Object[0]):(tList.size() < 4?null:(ItemStack)tList.get(3)), Math.max(1, Math.abs(aMaterial.getProtons() * 8 * (int)tItemAmount1)), Math.min(4, tList.size()) * 30);
               }

               if((aMaterial.mExtraData & 2) != 0) {
                  GregTech_API.sRecipeAdder.addCentrifugeRecipe(GT_Utility.copyAmount(tItemAmount1, new Object[]{aStack}), tCapsuleCount > 0L?(int)tCapsuleCount:0, (ItemStack)tList.get(0), tList.size() < 2?null:(ItemStack)tList.get(1), tList.size() < 3?null:(ItemStack)tList.get(2), tCapsuleCount < 0L?GT_Items.Cell_Empty.get(-tCapsuleCount, new Object[0]):(tList.size() < 4?null:(ItemStack)tList.get(3)), Math.max(1, Math.abs(aMaterial.getMass() * 2 * (int)tItemAmount1)));
               }
            }
         }
      }
